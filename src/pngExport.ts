...
export class PngEncodingSession {
  ...
  private constructor(worker: Worker) {
    ...
    worker.onerror = (event) => {
      const error = new Error(event.message || "PNG 编码线程启动失败");
      this.pending.forEach(({ reject }) => reject(error));
      this.pending.clear();
    };
  }
  ...
  static async create(width: number, height: number) {
    const session = new PngEncodingSession(
      new Worker(new URL("./workers/pngEncoder.worker.ts", import.meta.url), {
        type: "module",
      }),
    );
  ...
  async addStrip(imageData: ImageData) {
    ...
    this.worker.postMessage(
      {
        ...
        id,
        rgba,
        stripWidth: imageData.width,
        stripHeight: imageData.height,
      },
      [rgba],
    );
    await done;
  ...
  async finish() {
    ...
    if (!bytes) throw new Error("PNG 编码结果为空");
